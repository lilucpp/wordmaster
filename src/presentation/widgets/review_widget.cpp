...
void ReviewWidget::setupUI() {
  ...
  wordLabel_->setStyleSheet(
      "font-size: 48px; font-weight: bold; color: #1976d2;");
  ...
  auto *hintLabel = new QLabel(
      "• 遗忘：1天后  • 困难：依然增长  • 良好：正常间隔  • 简单：大幅增长",
      this);
...
void ReviewWidget::startReviewSession() {
  ...
  session_ = service_->startSession(
      bookId_, Application::StudyService::StudySession::Review,
      50 // 每次复习最多50个
  );
